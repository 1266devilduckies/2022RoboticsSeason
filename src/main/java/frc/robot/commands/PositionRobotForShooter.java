...
public class PositionRobotForShooter extends CommandBase {
  ...
  //Proportional Gain | Feed-Forward Gain
  Double[][] pfConstants = {
    {0.02, 0.045}, //0.5
    {0.03, 0.05} //3.0
  };
  ...
  public void initialize() {
    ...
    for (HashMap.Entry<Double, Double[]> entry : shooterDistances.entrySet()) {
      ...
      if (Math.abs(key-distance) < nearest) {
        nearest = key;
  ...
  @Override
  public void execute() {
    distance = getLimelightDistance();
    double pidOutput = movementController.calculate(distance, nearest);
  ...
  public boolean isFinished() {
    return (Math.abs(distance-nearest) < 0.1) || getLimelightDistance() < 0 /*limelight cant see top*/;
  }
